...
public class TorrentTest {
    ...
    public void testTorrentContents() throws IOException {
        ...
        assertEquals("st_exupery_le_petit_prince.pdf", torrent.getName());
        assertEquals("Le petit prince est l'oeuvre la plus connue d'Antoine de Saint-Exupery", torrent.getComment());
        assertEquals("Transmission/2.20b3 (11793)", torrent.getAuthor());
        ...
        List<TrackerInfo> trackers = torrent.getTrackers();
        assertNotNull(trackers);
        assertFalse(trackers.isEmpty());
        assertEquals(2, trackers.size());
        assertEquals("http://icsinsrv1.epfl.ch:6969/", trackers.get(0).getUrl());
        assertEquals("http://127.0.0.1:6969/", trackers.get(1).getUrl());
    }

    @Test
    public void testReadFromFileGivenDataFile() throws IOException {
        Torrent torrent = new Torrent(new File("data/LePetitPrince.torrent"), new File("data/st_exupery_le_petit_prince.pdf"), false);
    ...
    public void testReadFromFileNoDataFile() throws IOException {
        Torrent torrent = new Torrent(new File("data/LePetitPrince.torrent"), null, false);
        torrent.setWritingEnabled(false);
    ...
    public void testReadFromFileWrongDataFile() throws IOException {
        Torrent torrent = new Torrent(new File("data/LePetitPrince.torrent"), new File("data/LePetitPrince.torrent"), false);
        torrent.setWritingEnabled(false);
