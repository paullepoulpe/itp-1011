...
public class PieceTest {
	...
	public static void setup() {
		try {
			...
			throw new RuntimeException(
					"Your Java runtime doesn't support SHA-1 digests - that's incredible but we can't continue.",
					e);
		}
	}
	...
	public void testIllegalBegin1(Piece piece)
			throws UnsupportedEncodingException {
		setupPiece(Piece.BLOCK_SIZE + 1).feed(Piece.BLOCK_SIZE - 1,
				new byte[Piece.BLOCK_SIZE]); // invalid begin
	}

	@Test(expected = IllegalArgumentException.class)
	public void testIllegalBegin2() throws UnsupportedEncodingException {
		setupPiece(Piece.BLOCK_SIZE + 1).feed(-1, new byte[Piece.BLOCK_SIZE]); // index
		// cannot
		// be
		// negative
	}
	...
	public void testIllegalBegin3() throws UnsupportedEncodingException {
		setupPiece(Piece.BLOCK_SIZE + 1).feed(2 * Piece.BLOCK_SIZE,
				new byte[Piece.BLOCK_SIZE]); // index cannot be too big
	}

	@Test(expected = IllegalArgumentException.class)
	public void testIllegalBlockLength1() throws UnsupportedEncodingException {
		setupPiece(Piece.BLOCK_SIZE + 1)
				.feed(0, new byte[Piece.BLOCK_SIZE / 2]); // invalid block
		// length
	}
	...
	public void testIllegalBlockLength2() throws UnsupportedEncodingException {
		setupPiece(Piece.BLOCK_SIZE + 1)
				.feed(0, new byte[Piece.BLOCK_SIZE + 1]); // invalid block
		// length
	}

	@Test(expected = IllegalArgumentException.class)
	public void testIllegalBlockLength3() throws UnsupportedEncodingException {
		setupPiece(Piece.BLOCK_SIZE + 1).feed(Piece.BLOCK_SIZE,
				new byte[Piece.BLOCK_SIZE + 1]);
		// invalid begin or block length
	}
}
