...
public class RSAEncodeDecodeTest {
	 ...
	 */
	/*
	 * Decomment this test if your KeyPair class throws an exception when the
	 * size of the key is not a multiple of 8
	 * 
	 * @Test(expected = IllegalArgumentException.class)
	 */
	public void testBadModulusSize() {
		new KeyPair(new BigInteger("3233"), new BigInteger("17"),
				new BigInteger("2753"), 20);
	}

	@Test
	public void testKeyPair() {
		KeyPair keyPair = new KeyPair(new BigInteger("3233"), new BigInteger(
				"17"), new BigInteger("2753"), 16);
		...
		keyPair = new KeyPair(new BigInteger("17947"), new BigInteger("3"),
				new BigInteger("11787"), 16);
	...
	public void testEncryptDecrypt() throws Exception {
		...
		for (int pow = 3; pow < 11; pow++) {
			...
			assertEquals(
					"The original message must be equal to the encrypted-decrypted message",
					decryptedMessage, message);
		}
	}

	private BigInteger[] encrypt(KeyPair keyPair, byte[] input)
			throws UnsupportedEncodingException {
		BigInteger[] output = new BigInteger[input.length];

		for (int i = 0; i < input.length; i++) {
			output[i] = keyPair.encrypt(new BigInteger(Integer
					.toString(input[i])));
		}
	...
	private byte[] decrypt(KeyPair keyPair, BigInteger[] input)
			throws UnsupportedEncodingException {
